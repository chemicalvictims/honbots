...
function object:oncombateventOverride(EventData)
    ...
    local nAddBonus = 0
     if EventData.Type == "Ability" then
        if EventData.InflictorName == "Ability_Deadwood1" then
            nAddBonus = nAddBonus + object.nRootUse
        elseif EventData.InflictorName == "Ability_Deadwood4" then
            nAddBonus = nAddBonus + object.nPunchUse
        end
...
local function HarassHeroExecuteOverride(botBrain)
    ...
    local abilRoot = skills.abilQ
    local abilLog = skills.abilW
    local abilPunch = skills.abilR
    ...
    --- if an ability command has been given successfully
        ...
        if not bActionTaken and nLastHarassUtility > botBrain.nRootThreshold then
			if bDebugEchos then BotEcho("  No action yet, checking root") end
			--ROOT
			if abilRoot:CanActivate() then
				local nRange = abilRoot:GetRange()
				if nTargetDistanceSq < (nRange * nRange) then
					...
					bActionTaken = core.OrderAbilityPosition(botBrain, abilRoot,(vecTargetPosition+relativeMov), false, true)
				end
			end
		end
		...
		if core.CanSeeUnit(botBrain, unitTarget) then		
			--FALCONPUNCH
			if not bActionTaken and nLastHarassUtility > botBrain.nPunchThreshold then
				--graveyard could take up to 500ms after it is cast to stun, so wait at least that long if we just cast it
				if bDebugEchos then BotEcho("  No action yet, checking punch") end
				if abilPunch:CanActivate() then
					local nRange = abilPunch:GetRange()
					if nTargetDistanceSq < (nRange * nRange) then
						bActionTaken = core.OrderAbilityEntity(botBrain, abilPunch, unitTarget)
					end
				end
			end
		end   
...
BotEcho('finished loading dw_main')